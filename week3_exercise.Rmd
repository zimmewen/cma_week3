...
ggplot(caro, aes(x=stepMean))+         
  geom_histogram(binwidth=2) #zeigt meiste Werte liegen bei 50-10
```
...
caro <- caro %>% 
  ungroup() %>%
  mutate(static = stepMean < mean(5, na.rm = TRUE)) #'5' ist der gewählte Threshold
...
## Task 3: Visualize segmented trajectories spatially
```{r}
