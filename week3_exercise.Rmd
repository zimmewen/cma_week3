...
#Remove static values by assigning threshold
```{r}
caro <- caro %>% 
#  ungroup() %>%    #nicht ganz klar warum man hier wieder entgruppiert wird
  mutate(static = stepMean < mean(5, na.rm = TRUE)) #gewählte threshold '5' als Boolean in neuer Spalte ablegen

#caro_threshold <- caro %>%
#  filter(static == FALSE)  #im Beispiel stand "filter(!static)", um doppelte Verneinung zu vermeiden 
...
```{r}
ggplot(caro, aes(E, N, color=static))+   #Achtung, hier gesamten Datensatz nehmen
  geom_path()+
  geom_point()+
  coord_equal()
...
```{r}
caro %>%
  filter(static == FALSE) %>%   #Filter, damit nur Segmente in denen sich etwas bewegt, gezeigt werden
